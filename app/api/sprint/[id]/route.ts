import type { SprintPatchRequest } from "@/types";
...
/** GET /api/sprint/:id → SprintDetail (P12 render; additive contract row). */
...
/** EP-021 — PATCH /api/sprint/:id (SprintPatchRequest) → SprintDetail.
 * The locked baseline is immutable — tamper attempts are rejected loudly. */
export async function PATCH(
  ...
  try {
    ...
    if (body.change_after !== undefined) {
      if (typeof body.change_after !== "string" || body.change_after.length > 4000) {
        return err("VALIDATION_ERROR", "change_after must be a string (≤4000)");
      }
      patch.change_after = body.change_after;
    }
    if (body.add_custom_task !== undefined) {
      const c = body.add_custom_task as Record<string, unknown>;
      if (typeof c?.title !== "string" || typeof c?.rubric_key !== "string") {
        return err("VALIDATION_ERROR", "add_custom_task needs { title, rubric_key }");
      }
      patch.add_custom_task = { title: c.title, rubric_key: c.rubric_key };
    }
